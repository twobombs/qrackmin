...
# Create or clear the error log file at the start
> errors_ising_qbdd_fixes.txt
...
for ((i=4; i<=56; i++)); do
  ...
  for ((j=4; j<=i; j++)); do
    ...
    python3 ising_qbdd_fixes.py "$j" "$i" 2>>errors_ising_qbdd_fixes.txt 1>>ising_qbdd_fixes_$(echo $j).txt &
